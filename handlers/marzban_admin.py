...
async def open_marzban_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(f"📦 نگاشت پلن‌ها به پنل فعال{warn}", reply_markup=admin_marzban_menu())
...
async def menu_admin_marzban(message: types.Message):
    if not _admin_perm(message.from_user.id, "vpn_panel"):
        return
    warn = _marzban_hub_warn()
    await message.answer(f"📦 نگاشت پلن‌ها به پنل فعال{warn}", reply_markup=admin_marzban_menu())
