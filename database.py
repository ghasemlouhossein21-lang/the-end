...
def init_db():
    ...
    # مهاجرت دعوت‌ها: مبلغ پاداش‌های pending نسخه‌های قدیمی را به ۵۰,۰۰۰ تومان
    # می‌رسانیم و دعوت‌هایی را که فرد دعوت‌شده قبلاً اولین خرید واقعی و پولی داشته
    # ولی هنوز آزاد نشده‌اند، همان‌جا تکمیل می‌کنیم.
...
def set_text_override(key: str, text: str) -> None:
    _ensure_text_overrides_table()
    with transaction() as cur:
        cur.execute(
            "INSERT INTO text_overrides(key, text, updated_at) VALUES (?, ?, ?) "
...
def list_text_overrides() -> list[dict]:
    _ensure_text_overrides_table()
    cur = get_connection().cursor()
    cur.execute("SELECT key, text, updated_at FROM text_overrides ORDER BY key")
    return _fetchall(cur)
...
def export_backup_json(path: str):
    ...
    tables = ["users", "transactions", "configs", "discounts", "discount_usages", "agents",
              "referrals", "orders", "vip_categories", "vip_plans", "text_overrides"]
    data = {}
    for table in tables:
